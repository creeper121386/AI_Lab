...
trainLoader = torch.utils.data.DataLoader(
    dataset=trainData, batch_size=batchSize, shuffle=True)
...
fixed_noise = torch.randn(
    batchSize, nz, 1, 1).cuda()               # 用来生成假图片的噪声
...
for j in range(epoch):
    for i, (img, _) in enumerate(trainLoader, 0):
        ...
        # 为所有label标记1（要使得造假后的图片输入D后，输出的结果更接近1）
        label.fill_(1)
        ...
        if not(i % 100):
            ...
            torchvision.utils.save_image(
                real, '%s/realSamples.png' % path, normalize=True)
